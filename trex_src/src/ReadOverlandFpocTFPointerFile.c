/*---------------------------------------------------------------------
C-  Function:   ReadOverlandFpocTFPointerFile.c
C-
C-  Purpose\    ReadOverlandFpocTFPointerFile is called to read the
C-  Methods:    cell values of time function ID (pointer) values that
C-              link cells in the domain to specific ocerland fpoc
C-              time series functions.
C-
...
void ReadOverlandFpocTFPointerFile(int isolid, int ilayer)
{
	...
	printf("\n\n*******************************************************\n");
	printf(	   "*                                                     *\n");
	printf(	   "*   Reading Overland Fpoc Time Function Pointer File  *\n");
	printf(	   "*                                                     *\n");
	printf(	   "*******************************************************\n\n\n");
	...
	//Write label for fpoc file to file
	fprintf(echofile_fp, "\n\n\n  Overland Fpoc Time Function ID File: solid = %d  \n", isolid);
	fprintf(echofile_fp,       "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	...
	if(gridrows != nrows || gridcols != ncols || cellsize != dx)
	{
		...
		fprintf(echofile_fp, "\n\n\nOverland Fpoc TFPointer File Error:\n");
		...
		printf("Overland Fpoc TFPointer File Error:\n");
	...
	fprintf(echofile_fp, "\nOverland Fpoc TFPointer Characteristics: solid = %d\n", isolid);
	...
	for(i=1; i<=gridrows; i++)
	{
		...
		for(j=1; j<=gridcols; j++)
		{
			...
			if(imask[i][j] > 0)
			{
				...
				fpocovtfid[isolid][i][j][ilayer] = tfid;
