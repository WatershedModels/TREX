/*---------------------------------------------------------------------
C-  Function:   Read3DIntegerGrid.c
C-
C-  Purpose/    Read3DInteger Grid is called at the start of the
...
void Read3DIntegerGrid(char name[], int ***gridvalue, int ilayer)
{
...
*/
	...
	for(i=1; i<=gridrows; i++)
	{
		...
		{
			//Record 3
			fscanf(gridfile_fp, "%d", &gridvalue[i][j][ilayer]);	//read integer value from grid
/*
			//Echo value in overland cell to file
			fprintf(echofile_fp, "  %5d", gridvalue[i][j][ilayer]);
*/
