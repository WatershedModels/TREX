/*---------------------------------------------------------------------
C-  Function:   Write3DIntegerGrid.c
C-
C-  Purpose/    Write3DInteger Grid is called at the end of the
C-  Methods:    simulation to write initial condition integer (int)
...
void Write3DIntegerGrid(char name[], int ***gridvalue, int ilayer)
{
	...
	for(i=1; i<=nrows; i++)
	{
		...
		for(j=1; j<=ncols; j++)
		{
			...
			if(imask[i][j] != nodatavalue)
			{
				...
				fprintf(gridfile_fp,"%d ",gridvalue[i][j][ilayer]);
			}
