...
void ReadDataGroupC()
{
	...
	if(chnopt > 0)
	{
		...
		//else if the channel erosion option > 1
		else if(erschopt > 1)
		{
			//Write label for channel erosion parameters
	...
	for(i=1; i<=nsolids; i++)
	{
		...
		if(chnopt > 0)
		{
			//if the channel erosion option = 1 (transport capacity limited erosion)
			if(erschopt == 1)
			{
				//Record 8f
				fscanf(inputfile_fp, "%f",	//read
			...
			//else if the channel erosion option > 1 (erosion rate equation)
			else if(erschopt > 1)
			{
				//Record 8g
				fscanf(inputfile_fp, "%f %f",	//read
...
*/				
	...
	nmanningov = (float *)malloc((nlands+1) * sizeof(float));			//Manning's n (overland plane)
	...
	if(ersovopt <= 1)
	{
		...
		for(i=1; i<=nlands; i++)
		{
			//Record 24 (Part 1)
			fscanf(inputfile_fp, "%f %f %f %f",		//read
				&nmanningov[i],			//Manning's n for this land use class
			...
			fprintf(echofile_fp, "%9.5f  %22.3f  %5.3f  %5.3f %s",
				nmanningov[i],
				interceptionclass[i],
				cusle[i],
				pusle[i],
