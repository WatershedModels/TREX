/*---------------------------------------------------------------------
C-  Function:   Write2DFloatGrid.c
C-
C-  Purpose/    Write2DFloatGrid is called at the end of the
C-  Methods:    simulation to write initial condition real (float) grid
...
void Write2DFloatGrid(char name[], float **gridvalue)
{
	...
	for(i=1; i<=nrows; i++)
	{
		...
		for(j=1; j<=ncols; j++)
		{
			...
			if(imask[i][j] != nodatavalue)
			{
				...
				fprintf(gridfile_fp,"%e ",gridvalue[i][j]);
			}
