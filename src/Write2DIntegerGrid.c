/*---------------------------------------------------------------------
C-  Function:   Write2DIntegerGrid.c
C-
C-  Purpose/    Write2DInteger Grid is called at the end of the
...
void Write2DIntegerGrid(char name[], int **gridvalue)
{
	...
	for(i=1; i<=nrows; i++)
	{
		...
		for(j=1; j<=ncols; j++)
		{
			...
			if(imask[i][j] != nodatavalue)
			{
				...
				fprintf(gridfile_fp,"%d ",gridvalue[i][j]);
			}
